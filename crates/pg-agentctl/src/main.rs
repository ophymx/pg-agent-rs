...
enum Cmd {
    ...
    CheckHooks {
        ...
        /// Check against the pre-cutover (pgpool-led) hook block
        /// instead of the agent-led contract.
        #[arg(long)]
        legacy: bool,
    },
    ...
    GenPgpool {
        ...
        /// Emit the pre-cutover (pgpool-led) hook block instead of the
        /// agent-led contract.
        #[arg(long)]
        legacy: bool,
    },
...
async fn dispatch(cli: Cli) -> anyhow::Result<ExitCode> {
    match cli.cmd {
        ...
        Cmd::CheckHooks { path, legacy } => check_hooks(&path, legacy, cli.json),
        Cmd::GenPgpool {
            write,
            config,
            legacy,
        } => gen_pgpool(config, write, legacy, cli.socket.as_deref(), cli.json).await,
...
async fn gen_pgpool(
    ...
    let fragment = render_pgpool_fragment(&rows, legacy);
...
fn render_pgpool_fragment(rows: &[BackendRow], legacy: bool) -> String {
    ...
    const KEY_WIDTH: usize = 26;
    if legacy {
        s.push_str("# --- canonical hook block (LEGACY: pgpool-led failover) ---\n");
        for h in hookspec::pgpool_hooks_legacy() {
...
fn check_hooks(path: &std::path::Path, legacy: bool, json: bool) -> anyhow::Result<ExitCode> {
...
mod tests {
    ...
    fn render_pgpool_fragment_emits_backend_block_and_hooks() {
        ...
        let out = render_pgpool_fragment(&rows, false);
        ...
        // Legacy keeps the pre-cutover shape and no settings block.
        let legacy = render_pgpool_fragment(&rows, true);
        assert!(legacy.contains("wd_escalation_command"));
        assert!(legacy.contains("follow_primary_command     = 'pg_agentc follow_primary"));
        assert!(!legacy.contains("use_watchdog"));
    }
...
quoted_with_spaces = '  spaces inside  '
    ...
    fn check_hooks_reports_drift_when_value_mismatched() {
        ...
        let code = check_hooks(&p, false, false).unwrap();
        assert_eq!(
            format!("{code:?}"),
    ...
    /// The two contracts are distinguishable: the agent-led conf fails
    /// the legacy check (follow hook empty, wd hooks absent) and vice
    /// versa — which is what makes `--legacy` a real flag rather than
    /// a synonym.
    #[test]
    fn check_hooks_legacy_dissents_on_the_agent_led_conf() {
        let mut text = String::new();
        for h in hookspec::pgpool_hooks() {
            text.push_str(&format!("{} = '{}'\n", h.key, h.value));
        }
        ...
        let p = tmp.path().join("pgpool.conf");
        std::fs::write(&p, text).unwrap();
        let code = check_hooks(&p, true, false).unwrap();
        assert_eq!(
            format!("{code:?}"),
            format!("{:?}", ExitCode::FAILURE),
            "legacy check must reject the agent-led conf"
        );
    }
